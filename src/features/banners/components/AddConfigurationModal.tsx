...
const ConfigureActionsModal: React.FC<ConfigureActionsModalProps> = ({
  ...
  const handleSave = () => {
    if (actionType && selectedValue) {
      ...
      if (secondaryOptions && secondaryOptions.options.length > 0) {
      ...
      onSave({ actionType, selectedValue, selectedLabel });
      onClose();
    }
  };
          ...
          <>
            ...
            <div className="flex shrink-0 items-center justify-end gap-3 px-6 py-4 border-t border-body-content/20">
              ...
              <Button
                variant="filled"
                onClick={handleSave}
                disabled={!actionType || (!selectedValue && secondaryDropdown?.type !== "placeholder")}
                className="px-8"
              >
                Save
              </Button>
