...
const colorMap: Record<ChipColor, { bg: string; text: string }> = {
  ...
  pink: {
    bg: "bg-pink-50 dark:bg-pink-950/80",
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex w-fit items-center justify-center rounded-lg px-1.5 py-1",
          bg,
          className,
        )}
        {...props}
      >
        <p className={cn("text-xs font-semibold", text, labelClassName)}>{label}</p>
        {isCollapsible && (
          <Icon
            name={closeIcon}
            size={16}
            onClick={onCollapse}
            className={cn("ml-1 cursor-pointer", iconClassName)}
