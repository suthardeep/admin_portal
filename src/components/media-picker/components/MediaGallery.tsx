...
import { useFoldersApi, useFilesApi, useUploadFiles, useVendorsApi } from "@/components/media-picker/api/queryHooks";
import { MediaItem, Vendor } from "@/components/media-picker/types/media.types";
import { PlatformType } from "@/components/media-picker/types/media.api";
...
export const MediaGallery = ({
  ...
  // Platform and Vendor State
  const [platformType, setPlatformType] = useState<PlatformType>("admin");
  ...
  const { data: folderData, isLoading: loadingFolders } = useFoldersApi(folderParams);
  ...
  // Vendors query (only when platform type is vendor)
  const {
    data: vendorPages,
    isLoading: loadingVendors,
    fetchNextPage: fetchNextVendorPage,
  ...
  // Flatten vendors from pages
  const vendors = useMemo(() => {
    return vendorPages?.pages.flatMap((page) => page.data) || [];
  }, [vendorPages]);
  ...
  const displayItems = useMemo(() => {
    if (currentFolder) {
      ...
      let items = folders;
      if (searchQuery) {
        items = items.filter((folderName: string) => folderName.toLowerCase().includes(searchQuery.toLowerCase()));
      }
      return items;
    }
  ...
  const handlePlatformTypeChange = (value: any) => {
    if (value === '' || value === null) {
      // Platform dropdown cleared - reset to admin
      setPlatformType("admin");
      setSelectedVendor(null);
    } else {
      setPlatformType(value as PlatformType);
      setSelectedVendor(null);
    }
    setCurrentFolder(null);
    setSelection(new Set());
  ...
  const handleLoadMoreVendors = async () => {
    if (hasMoreVendors && !isFetchingNextVendorPage) {
      await fetchNextVendorPage();
    }
  };
        ...
        {activeTab === "browser" && (
          <div className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* Platform Type Dropdown - same height as search */}
                  <div className="w-72">
                    <Dropdown
                  ...
                  {/* Vendor Dropdown - only show when vendor is selected, same height as search */}
                  {platformType === "vendor" && (
                    <div className="w-72">
                      <Dropdown
                        ...
                        isLoading={loadingVendors || isFetchingNextVendorPage}
                        loadMoreText="Load more vendors"
            ...
            <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400">
              {platformType === "vendor" && !selectedVendor ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 py-16">
                  ...
                  <p className="text-lg font-semibold text-gray-700">Please select a vendor</p>
                  <p className="text-sm text-gray-500 mt-2">Choose a vendor from the dropdown above to view their media</p>
                </div>
